...
public class tanos 
{
	...
	{
		int mass[] = {10000, 1, 20, 5, 15, 1342, 0, 7, 9999, 76132}; // Подаем с главной функции, можно и с консоли, но вроде еще не проходили:)
		int min = 0; // Для обозначения индекса первого элемента части
		int max = (mass.length - 1); // Для обозначения индекса последнего элемента части

		mass = ft_sort(mass, min, max); // Сортирует
		mass = ft_min_max(mass, min, max); // Двигает индексы, деля массив на подчасти, которые тоже сортируем
		while(min < max + 1)
		{
			System.out.println(mass[min]);
			min++;
		}
    }
